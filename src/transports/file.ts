...
import type { Transport, LogInfo, LogLevel, LogFormat, FileTransportOptions } from '../types';
import * as format from '../format'; // Importuj format (dla defaultFileFormat)
// Importuj rotating-file-stream i potrzebne typy
import { createStream, RotatingFileStream, Generator } from 'rotating-file-stream';
...
export class FileTransport implements Transport {
    ...
    constructor(options: FileTransportOptions) {
        ...
        if (options.createPath !== false) {
            try {
                ...
                if (e.code !== 'EEXIST') {
                     console.error(`[scribelog] Error creating log directory for ${options.filename}:`, e);
                     throw e;
                }
            }
        }
        ...
        try {
             ...
             const streamOptions: RfsOptions = { // Użyj typu RfsOptions
             ...
             if (options.interval !== undefined) streamOptions.interval = options.interval;
             if (options.compress !== undefined) {
                 // Konwertuj boolean na string 'gzip' jeśli trzeba
                 streamOptions.compress = options.compress === true ? 'gzip' : options.compress;
             }
             if (options.maxFiles !== undefined) streamOptions.maxFiles = options.maxFiles;
             if (options.maxSize !== undefined) streamOptions.maxSize = options.maxSize;
            ...
            console.error(`[scribelog] Failed to create rotating file stream for ${options.filename}:`, err);
    ...
    log(processedEntry: Record<string, any> | string): void {
        ...
        if (!this.stream || !this.stream.writable || this.stream.destroyed) {
            console.error('[scribelog] FileTransport stream is not writable. Log entry dropped.', processedEntry);
            return;
        }
        ...
        if (typeof processedEntry === 'object' && processedEntry !== null) {
            try {
                 ...
                 output = (this.format || FileTransport.defaultFileFormat)(processedEntry) as string;
                 if (typeof output !== 'string') {
                     console.error('[scribelog] FileTransport formatter did not return a string. Using JSON.stringify fallback.', processedEntry);
                     output = JSON.stringify(processedEntry);
                 }
            } catch (formatErr) {
                 console.error('[scribelog] Error applying format in FileTransport:', formatErr, 'Original entry:', processedEntry);
                 try { output = JSON.stringify(processedEntry); } catch { output = "[scribelog] Failed to format or stringify log entry."}
            }
        } else if (typeof processedEntry === 'string') {
            output = processedEntry;
        } else {
            console.warn('[scribelog] FileTransport received non-object/non-string data:', processedEntry);
            return;
        }
        ...
        this.stream.write(output + '\n', (err) => {
            if (err) {
                ...
                console.error('[scribelog] FileTransport failed to write to stream:', err);
            }
        });
    }
