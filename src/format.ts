...
export const errors = (options: ErrorsOptions = {}): LogFormat => {
    ...
    return (info: Record<string, any>): Record<string, any> => {
        ...
        if (error && errorKey) {
             ...
             if (info.eventType === undefined && errorKey === 'error') info.eventType = 'caughtError'; // Oznacz błąd złapany przez loggera
...
export const hostname = (options: { alias?: string } = {}): LogFormat => {
    ...
    try { host = os.hostname(); } catch (e) { console.error('[scribelog] Failed to get hostname:', e); }
    return (info: Record<string, any>): Record<string, any> => {
        if (host !== undefined) { info[alias] = host; }
        return info;
    };
};
...
export const metadata = (options: { alias?: string; exclude?: string[] } = {}): LogFormat => {
  ...
  const standardAliases = [
    'timestamp', 'level', 'message', 'originalTimestamp', // Pola czasu i podstawowe
    'errorName', 'stack', // Pola błędu, które nie powinny być w meta, bo są obsługiwane inaczej
    'splat',           // Pole robocze splat
    'pid', 'hostname'  // Pola specjalne pid/hostname
...
export const simple = (options: SimpleOptions = {}): LogFormat => {
    ...
    const defaultLevelColors: Record<string, Chalk> = { // Użyj string jako klucza dla kompatybilności
        error: chalk.red, warn: chalk.yellow, info: chalk.green,
        http: chalk.magenta, verbose: chalk.cyan, debug: chalk.blue,
        silly: chalk.gray,
    };
    ...
    return (info: Record<string, any>): string => {
        ...
        const coloredLevel = shouldUseColors ? levelColorFn(`[${levelStr.toUpperCase()}]`) : `[${levelStr.toUpperCase()}]`;
        const coloredTimestamp = shouldUseColors ? timestampColorFn(timestampStr) : timestampStr;
        const pidStr = info.pid ? (shouldUseColors ? pidColorFn(`(pid:${info.pid})`) : `(pid:${info.pid})`) : '';
        const hostnameStr = info.hostname ? (shouldUseColors ? hostnameColorFn(`@${info.hostname}`) : `@${info.hostname}`) : '';

        const meta: Record<string, any> = {};
        // --- POCZĄTEK ZMIANY: Usunięto pola błędów z forbidden ---
        const forbidden = new Set(['timestamp', 'level', 'message', 'originalTimestamp', 'errorName', 'stack', 'splat', 'pid', 'hostname']);
        // Usunięto: 'originalReason', 'exception', 'eventType'
        // --- KONIEC ZMIANY ---
        for (const key in info) { if (typeof key !== 'symbol' && !forbidden.has(key) && Object.prototype.hasOwnProperty.call(info, key)) { meta[key] = info[key]; } }
...
export const defaultJsonFormat = combine( errors({ stack: true }), splat(), timestamp(), pid(), hostname(), level(), message(), metadata(), json() );
export const defaultSimpleFormat = combine( errors({ stack: true }), splat(), timestamp(), pid(), hostname(), level(), message(), metadata(), simple() );