...
describe('FileTransport', () => {
         ...
         try {
             ...
             for (const file of files) {
                 if (file.startsWith('test-') || file.startsWith('rotate-')) {
                     ...
                     catch (unlinkErr) { await delay(50); try { fs.unlinkSync(path.join(LOG_DIR, file)); } catch {} }
                 }
             }
         } catch (e) { /* Ignoruj */ }
    });
    ...
    it('should create a log file and write JSON entries by default', async () => {
        ...
        await delay(150);
        await new Promise<void>(resolve => { fileTransport?.close?.(); (fileTransport as any).stream.once('finish', () => resolve()); setTimeout(resolve, 500); });
        await delay(50);
    ...
    it('should use the specified format (e.g., simple)', async () => {
     ...
     it('should respect transport level', async () => {
        ...
        const fileTransport = new transports.File({ filename: logFile, level: 'error' });
        const logger = createLogger({ level: 'info', transports: [fileTransport] });
     ...
     it('should accept rotation options without throwing', () => {
         const options: FileTransportOptions = {
             ...
             size: '1K', interval: '1s', compress: 'gzip', maxFiles: 3
         };
          let transport: Transport | undefined;
         expect(() => { transport = new transports.File(options); }).not.toThrow();
         transport?.close?.();
     });
